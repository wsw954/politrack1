...
export async function getTrackedProvisionsForBill({
  userId,
  billId,
  page = 1,
  limit = 50,
  ...
  // ask API to include meta block
  params.set("with", "meta");

  const qs = params.toString() ? `?${params.toString()}` : "";

  const { meta, list, annotationsByProvisionId } = await request(
    `/api/users/${userId}/tracker/bills/${billId}/provisions${qs}`
  );

  return {
    list: list || [],
    meta: meta || { page: 1, pages: 1, total: 0, limit },
    annotationsByProvisionId: annotationsByProvisionId || {},
  };
}
