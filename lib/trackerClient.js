...
export async function getTrackedBill(
  ...
  const q = includeProvisions ? "?includeProvisions=1" : "";
  return request(`/api/users/${userId}/tracker/bills/${billId}${q}`);
}

/**
 * Whole-bill annotations PATCH payload shape (all optional):
 * {
 *   generalNotes?: string,
