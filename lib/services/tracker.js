...
export async function getProvisionAnnotations({ userId, billId, pid }) {
  await ensureDb();
  if (!isObjectId(userId) || !isObjectId(billId) || !isObjectId(pid))
    return null;
  ...
  const res = await User.updateOne(
    {
      _id: userId,
      "tracker.bills.itemId": billId,
      "tracker.bills.provisionAnnotations.pid": pid,
    },
    {
      $set: {
    ...
    },
    {
      arrayFilters: [
        ...
        { "prov.pid": new mongoose.Types.ObjectId(pid) },
      ],
    }
  );
...
export async function clearProvisionAnnotations({ userId, billId, pid }) {
  await ensureDb();
  if (!isObjectId(userId) || !isObjectId(billId) || !isObjectId(pid))
    return { ok: false, status: 400, message: "Invalid ids" };
...
export async function listAnnotatedProvisionsIndexed({
    ...
    pipeline.push(
      {
        $lookup: {
          from: "bills",
          let: { bid: new mongoose.Types.ObjectId(billId), pid: "$pid" },
          pipeline: [
  ...
  pipeline.push({ $sort: { updatedAt: -1, pid: 1 } });
  pipeline.push({
    $facet: {
