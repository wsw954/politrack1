// components/user/tracker/bills/FilterBar.js
"use client";

/**
 * Tracker-specific FilterBar for Bills
 * Mirrors the look/feel of components/bills/FilterBar, but scoped to tracked data
 * Adds room for future tracker-only filters (e.g., hasNote)
 */
export default function TrackerBillsFilterBar({
  allBills,
  filters,
  setFilters,
}) {
  const getOptions = (key) => {
    ...
    allBills.forEach((b) => {
      if (key === "title" && b.title) values.add(b.title);
      if (key === "status" && b.status?.current_stage)
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4 mb-6">
      ...
      {/* Has Note (tracker-only, future-friendly) */}
      <div className="col-span-2 sm:col-span-1 lg:col-span-1 bg-white border border-neutral-light rounded-lg shadow-sm p-3">
        <label
          htmlFor="hasNote"
          className="text-sm font-semibold mb-1 block text-neutral-dark"
        ...
        <select
          ...
          onChange={(e) => setFilters({ ...filters, hasNote: e.target.value })}
        >
